...
import { validateEnvironmentForInit, detectEnvironment } from '../utils/environment.js';
...
export async function initCommand() {
  ...
  p.log.step(`Workspace type: ${env.workspaceType || 'none (will use generic)'}`);
  ...
  if (existingConfig.repositories.length > 0) {
    p.log.info(`Found existing configuration with ${existingConfig.repositories.length} repositories`);
  }

  const group = await p.group({
    defaultDirectory: () => p.text({
