...
import { getAllRepositories, getRepository, addRepository } from '../utils/config.js';
import { updateSubtree, getBranches } from '../utils/git.js';

export async function updateCommand(name?: string, options: UpdateOptions = {}) {
  ...
  if (name && !options.all) {
    ...
    targetRepos = repositories.filter(repo => repoChoices.includes(repo.name));
  }
